...
func NewClient(provider, readerAddr string, poolAddres []string) *Client {
	var err error
	...
	client := &Client{
		...
		poolAddres: poolAddres,
	}
	return client
}

func (client *Client) GetLiquidityPoolStorage() map[string]*model.LiquidityPoolStorage {
	storages := make(map[string]*model.LiquidityPoolStorage)
	...
	for _, poolAddr := range client.poolAddres {
		liquidityPool := ethCommon.HexToAddress(poolAddr)
